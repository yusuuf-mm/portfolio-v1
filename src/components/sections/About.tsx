...
const NeuralNetwork = dynamic(() => import('@/components/ui/NeuralNetwork'), {
  ssr: false,
  loading: () => (
    <div className="w-full h-[400px] rounded-xl border border-[var(--border)] bg-[var(--glass-bg)] flex items-center justify-center">
...
export default function About() {
  ...
  // Scroll-based progress for neural network activation
  ...
  // Show terminal after network completes
  useEffect(() => {
    if (networkComplete) {
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-16 relative">
        ...
        >
          ...
          <p className="font-sans text-[var(--text-muted)] mt-4 max-w-2xl">
            Watch how software engineering foundations compound into AI systems expertise.
            Scroll to activate the forward pass.
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="rounded-xl border border-[var(--border)] bg-[var(--glass-bg)] backdrop-blur-md overflow-hidden">
            {/* Terminal header */}
            ...
            <div className="p-4 font-mono text-sm space-y-1">
              {terminalLines.map((line, i) => (
                <motion.div
                  ...
                  className={cn(
                    'flex gap-2',
                    line.highlight && 'text-bronze'
                  )}
                >
                  {line.prompt && (
                    <span className={cn(
                      line.highlight ? 'text-bronze' : 'text-[var(--text-muted)]'
                    )}>
                      {line.prompt}
                    </span>
                  )}
                  <span className={cn(
                    line.highlight ? 'text-bronze font-semibold' : 'text-[var(--text-secondary)]'
                  )}>
                    {line.text}
                  </span>
                </motion.div>
